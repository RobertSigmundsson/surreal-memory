...
Surreal-Memory exposes 53 tools via the [Model Context Protocol](https://modelcontextprotocol.io/):
...
| `smem_consolidate` | Run memory lifecycle (decay, prune, merge) |
...
*Surreal-Memory is open source and contributions are welcome. The spreading activation approach is particularly interesting if you've worked with cognitive architectures (ACT-R, Soar) — it's the same theoretical foundation applied to AI agent memory.*
