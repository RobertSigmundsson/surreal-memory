...
Surreal-Memory includes a self-answering documentation chatbot powered by spreading activation — no LLM required.

**Try it live:** [HuggingFace Space](https://huggingface.co/spaces/nhadaututtheky/surrealmemory-docs)
...
The chatbot uses `ReflexPipeline` — the same retrieval engine behind `smem_recall`.

## Running locally

```bash
pip install surreal-memory gradio
...
| Flag | Description |
|------|-------------|
| `--brain NAME` | Custom brain name (default: `surrealmemory-docs`) |
