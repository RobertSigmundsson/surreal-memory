...
Best practices and known risks when using Surreal-Memory.
...
smem check "Bob's phone: 555-1234"

# Use tags to mark confidence
smem remember "Bob's phone: 555-1234" -t unverified -t needs-confirmation

# Include source attribution
smem remember "Per email from Bob on 2024-02-04: API endpoint is /v2" --type fact
...
smem check "AWS_SECRET_KEY=xxx"
# ⚠️ SENSITIVE CONTENT DETECTED

# Auto-redact sensitive content
smem remember "Config: API_KEY=sk-xxx" --redact
...
smem recall "critical config" --json | jq '.confidence'

# Set minimum threshold
smem recall "important decision" --min-confidence 0.7

# Check memory age
smem recall "api endpoint" --show-age
```
...
```bash
smem brain create work-public      # Safe to share
smem brain create work-internal    # Internal only
...
```bash
smem brain export --exclude-sensitive -o shareable.json
smem brain health --name work-public
```
...
- Manual cleanup: `smem cleanup --expired`
- Use `--fresh-only` for context
- Apply decay: `smem decay`
...
**Resolved in v1.5.0:** The `smem_conflicts` tool detects factual contradictions and decision reversals at encode time, with manual resolution via `keep_existing`, `keep_new`, `keep_both` strategies.

## Incident Response

### Sensitive Data Stored

1. **Identify:** `smem brain health`
2. **Remove:** `smem brain delete compromised-brain --force`
