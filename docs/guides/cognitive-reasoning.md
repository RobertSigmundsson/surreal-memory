...
Surreal-Memory's Cognitive Reasoning Layer lets agents form hypotheses, gather evidence, make predictions, and verify outcomes — a structured scientific reasoning loop built on top of the memory graph.

## Quick Start

```
smem_hypothesize → smem_evidence → smem_predict → smem_verify
       ↑                                              |
       └──────── smem_schema (evolve) ←───────────────┘
```
...
- **smem_cognitive** — Dashboard: hot index + calibration score
- **smem_gaps** — Track what you *don't* know
- **smem_explain** — Trace connections between concepts
...
The hot index ranks items by urgency. `smem_cognitive(action="summary")` returns the top 20.
...
smem_cognitive(action="refresh")  # Recompute rankings
smem_cognitive(action="summary")  # View dashboard
```
...
|-------------|-----------|------------|
| `EVIDENCE_FOR` | evidence → hypothesis | `smem_evidence(type="for")` |
| `EVIDENCE_AGAINST` | evidence → hypothesis | `smem_evidence(type="against")` |
| `PREDICTED` | prediction → hypothesis | `smem_predict(hypothesis_id=...)` |
| `VERIFIED_BY` | prediction → observation | `smem_verify(outcome="correct")` |
| `FALSIFIED_BY` | prediction → observation | `smem_verify(outcome="wrong")` |
| `SUPERSEDES` | new hypothesis → old | `smem_schema(action="evolve")` |

These synapses are traversable via `smem_explain` — you can trace the full reasoning chain from evidence through hypothesis to prediction to verification.
...
5. **Evolve, don't delete.** When a hypothesis is partially wrong, use `smem_schema(evolve)` instead of creating a new one. This preserves the reasoning chain.

6. **Use `smem_gaps` proactively.** When you notice uncertainty, register it. Gaps surface in `smem_cognitive(summary)` so they don't get forgotten.
