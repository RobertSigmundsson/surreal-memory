...
**Migration is fully automatic.** When Surreal-Memory starts and detects schema version 20, it runs the v20→v21 migration:
...
```bash
pip install --upgrade surreal-memory
...
```bash
smem --verbose brain list 2>&1 | grep -i "migrat"
```
...
Then restart Surreal-Memory — it will re-run all migrations from scratch.
