...
Surreal-Memory is most useful when agents store durable context, not every passing detail. Use these guidelines to keep memory useful, auditable, and safe.
...
```bash
smem remember "Decision: keep update notices out of JSON output because agents parse CLI stdout as structured context. Source: issue triage on 2026-04-30." --type decision
```

Over:

```bash
smem remember "JSON output is fixed"
```
...
smem status --json
smem stats --json
smem recall "deployment constraints" --json
```
...
Use `smem check` for sensitive content before storing uncertain text:

```bash
smem check "candidate memory text"
```
...
smem recall "old decision" --show-age
```
