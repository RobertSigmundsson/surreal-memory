...
Surreal-Memory acts as a **unification layer** that imports, normalizes, and connects memories from multiple external memory systems into a single neural graph. Instead of managing fragmented knowledge across Mem0, Cognee, ChromaDB, Graphiti, LlamaIndex, and AWF separately, Surreal-Memory brings them together — preserving relationships, embeddings, and provenance while enabling cross-system recall via spreading activation.
...
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       ...
       └────────────┬────┴────┬────────────┘
              ...
              └─────────┬───────────┘
                        ▼
              ┌─────────────────────┐
              │   Surreal-Memory      │  ← Neurons, synapses, fibers
              │   (unified brain)   │
              └─────────────────────┘
```
...
3. **RecordMapper** runs each record through the Surreal-Memory encoder (creating neurons, synapses, fibers) and assigns TypedMemory with provenance
...
|--------|------|---------|---------------|-------------|
| **ChromaDB** | Vector DB | `ChromaDBAdapter` | `pip install surreal-memory[chromadb]` | fetch_all, embeddings, metadata, health |
| **Mem0** | Memory Layer | `Mem0Adapter` | `pip install surreal-memory[mem0]` | fetch_all, fetch_since, metadata, health |
| **Cognee** | Graph Store | `CogneeAdapter` | `pip install surreal-memory[cognee]` | fetch_all, relationships, metadata, health |
| **Graphiti** | Graph Store | `GraphitiAdapter` | `pip install surreal-memory[graphiti]` | fetch_all, fetch_since, relationships, metadata, health |
| **LlamaIndex** | Index Store | `LlamaIndexAdapter` | `pip install surreal-memory[llamaindex]` | fetch_all, embeddings, metadata, health |
...
from surreal_memory.integration.adapters import get_adapter
...
# Cognee graph edges become synapses in Surreal-Memory
...
External type strings map automatically to Surreal-Memory types:

| Source Type | Surreal-Memory Type | Source Systems |
|-------------|-------------------|----------------|
...
If no explicit type is provided, Surreal-Memory infers one from content using keyword heuristics.
...
External: (nodeA) --[caused_by]--> (nodeB)
    ↓
Surreal-Memory: (neuronA) --[CAUSED_BY, weight=0.5]--> (neuronB)
```
...
```python
from surreal_memory.integration.adapters import register_adapter
...
- Maps Mem0 categories to Surreal-Memory types
...
- Graph edges become `ExternalRelationship` instances → Surreal-Memory synapses
...
| Problem | Without Unification | With Surreal-Memory |
|---------|--------------------|--------------------|
| **Fragmented recall** | Query each system separately | Single `smem recall` searches all |
