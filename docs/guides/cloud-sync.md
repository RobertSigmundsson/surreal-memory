...
Sync your memories across devices using your own Cloudflare Worker. Your data stays on **your** Cloudflare account — Surreal-Memory never touches it.
...
```python
smem_remember("sensitive content", encrypted=True)
```

The Fernet key stays at `~/.surrealmemory/keys/{brain_id}.key` — never uploaded to the cloud.
...
npx wrangler d1 create smem  # Create your D1 database
...
smem_sync(action="seed")

# Push to cloud
smem_sync(action="push")

# Pull from cloud (on another device)
smem_sync(action="pull")

# Bidirectional sync
smem_sync(action="full")
```
...
If you're using Surreal-Memory as an MCP tool in Claude Code or Cursor, just run:

```python
smem_sync_config(action="setup")
```
...
When the same memory is modified on two devices, Surreal-Memory resolves conflicts using a configurable strategy:
...
```python
smem_sync_config(action="set", conflict_strategy="prefer_local")
```
...
```python
smem_sync_status()  # Shows device list with last sync time
```
