# Surreal-Memory for VS Code

A visual brain explorer, inline recall, and memory management extension for [Surreal-Memory](https://github.com/nhadaututtheky/surreal-memory) — the reflex-based memory system for AI agents.
...
Import memories from other systems into Surreal-Memory:
...
| Start Server | — | Start local Surreal-Memory server |
...
- [Surreal-Memory](https://pypi.org/project/surreal-memory/) v0.9.5+ (`pip install surreal-memory`)
- Python 3.11+
- A configured brain (auto-created on first use, or manually via `smem brain create my-brain && smem brain use my-brain`)

## Configuration

| Setting | Default | Description |
|---------|---------|-------------|
| `surrealmemory.pythonPath` | `"python"` | Python interpreter with surreal-memory installed |
| `surrealmemory.autoStart` | `false` | Auto-start the server on activation |
| `surrealmemory.serverUrl` | `"http://127.0.0.1:8000"` | Surreal-Memory server URL |
| `surrealmemory.graphNodeLimit` | `1000` | Max nodes in graph explorer (50-10000) |
| `surrealmemory.codeLensEnabled` | `true` | Show CodeLens hints for functions and comments |
| `surrealmemory.commentTriggers` | `["remember:", "note:", "decision:", "todo:"]` | Comment patterns that trigger encode suggestions |

## Getting Started

1. Install the extension
2. Install Surreal-Memory: `pip install surreal-memory`
3. Start the server: run **Surreal-Memory: Start Server** from the command palette, or enable `surrealmemory.autoStart`
5. Open the Surreal-Memory sidebar (brain icon in the activity bar)
...
If you find Surreal-Memory useful, consider buying me a coffee:
