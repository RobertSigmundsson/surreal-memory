...
/**
 * Manages the Surreal-Memory status bar item.
 *
...
export class StatusBarManager implements vscode.Disposable {
  ...
  constructor(private readonly _server: ServerLifecycle) {
    ...
    this._item.command = "surrealmemory.switchBrain";
    this._item.name = "Surreal-Memory";
  }
  ...
  private _render(): void {
    if (this._status === "starting") {
      this._item.text = "$(loading~spin) Surreal-Memory";
      this._item.tooltip = "Connecting to Surreal-Memory server...";
      return;
    }

    if (this._status === "disconnected") {
      this._item.text = "$(debug-disconnect) Surreal-Memory";
      this._item.tooltip = "Surreal-Memory: Disconnected. Click to connect.";
      this._item.command = "surrealmemory.connectServer";
      return;
    }
  ...
  private async _fetchStats(): Promise<void> {
    ...
    try {
      const client = new SurrealMemoryClient(this._server.baseUrl);
