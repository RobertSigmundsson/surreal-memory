...
import { SurrealMemoryClient } from "../server/client";
import { ServerLifecycle } from "../server/lifecycle";
import { StatusBarManager } from "../views/StatusBarManager";

const BRAINS_DIR = path.join(os.homedir(), ".surrealmemory", "brains");
const CONFIG_PATH = path.join(os.homedir(), ".surrealmemory", "config.toml");

/**
 * Read the current brain name from ~/.surrealmemory/config.toml.
 */
...
/**
 * List available brain names by scanning ~/.surrealmemory/brains/*.db.
 */
...
/**
 * Write the current brain name to ~/.surrealmemory/config.toml.
...
export function registerBrainCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("surrealmemory.switchBrain", async () => {
      if (!server.isRunning()) {
        vscode.window.showWarningMessage(
          "Surreal-Memory server is not running.",
        );
        return;
      }
      ...
      if (selected.action === "create") {
        await vscode.commands.executeCommand("surrealmemory.createBrain");
        return;
      }
  ...
  // Create Brain
  context.subscriptions.push(
    vscode.commands.registerCommand("surrealmemory.createBrain", async () => {
      if (!server.isRunning()) {
        vscode.window.showWarningMessage(
          "Surreal-Memory server is not running.",
        );
      ...
      try {
        const client = new SurrealMemoryClient(server.baseUrl);
