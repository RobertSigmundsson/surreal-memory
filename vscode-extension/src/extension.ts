...
export async function activate(
  ...
  const outputChannel =
    vscode.window.createOutputChannel("Surreal-Memory");
  context.subscriptions.push(outputChannel);
  outputChannel.appendLine("Surreal-Memory extension activating...");
  ...
  vscode.window.registerTreeDataProvider("surrealmemory.memories", memoryTree);
  ...
  outputChannel.appendLine("Surreal-Memory extension activated.");
...
function registerLifecycleCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("surrealmemory.startServer", async () => {
      if (server.isRunning()) {
        vscode.window.showInformationMessage(
          `Surreal-Memory server already running at ${server.baseUrl}`,
        );
        return;
      }
      try {
        ...
        vscode.window.showInformationMessage(
          `Surreal-Memory server started at ${server.baseUrl}`,
        );
      } catch (err) {
        statusBar.setStatus("disconnected");
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        const url = await vscode.window.showInputBox({
          prompt: "Enter Surreal-Memory server URL",
        ...
        await vscode.workspace
          .getConfiguration("surrealmemory")
        ...
        try {
          ...
          vscode.window.showInformationMessage(
            `Connected to Surreal-Memory at ${server.baseUrl}`,
          );
        } catch (err) {
          statusBar.setStatus("disconnected");
          vscode.window.showErrorMessage(
...
function registerGraphCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("surrealmemory.openGraph", () => {
