...
/**
 * Manages the Surreal-Memory server lifecycle with a detect-first,
...
export class ServerLifecycle implements vscode.Disposable {
  ...
  constructor(_context: vscode.ExtensionContext) {
    this._outputChannel = vscode.window.createOutputChannel("Surreal-Memory Server");
  ...
  async connectOrStart(): Promise<void> {
    ...
    const action = await vscode.window.showWarningMessage(
      "Surreal-Memory server not found. Start it or configure a URL.",
      "Start Server",
      "Configure URL",
    );
    ...
    if (action === "Start Server") {
      ...
      await vscode.commands.executeCommand(
        ...
        "surrealmemory.serverUrl",
      );
    }
  }

  /**
   * Spawn a new Surreal-Memory server process.
   */
  async start(): Promise<void> {
    ...
    statusItem.text = "$(loading~spin) Surreal-Memory: Starting...";
    statusItem.show();
    ...
    try {
      ...
      this._process.on("exit", (code, signal) => {
        ...
        if (code !== 0 && code !== null) {
          const msg = `Surreal-Memory server exited with code ${code}`;
          this._log(msg);
          this._notifyError(new Error(msg));
          this._offerRestart();
        } else if (signal) {
          this._log(`Surreal-Memory server killed by signal ${signal}`);
        }
      });
  ...
  private async _offerRestart(): Promise<void> {
    const action = await vscode.window.showErrorMessage(
      "Surreal-Memory server stopped unexpectedly.",
      "Restart",
      "Show Logs",
    );
  ...
  private _showSpawnError(err: Error): void {
    ...
    const message = isNotFound
      ? `Python not found at "${getConfig().pythonPath}". Install surreal-memory and configure surrealmemory.pythonPath.`
      : `Failed to start Surreal-Memory server: ${err.message}`;

    vscode.window.showErrorMessage(message, "Open Settings").then((action) => {
      if (action === "Open Settings") {
        vscode.commands.executeCommand(
          "workbench.action.openSettings",
          "surrealmemory.pythonPath",
        );
      }
    });
  }
  ...
  private _getLockFilePath(): string {
    const nmDir = path.join(os.homedir(), ".surrealmemory");
