...
const GITHUB_REPO = "surreal-memory";
const GITHUB_API_URL = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/releases/latest`;
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // 24 hours
const STATE_KEY_LAST_CHECK = "surrealmemory.lastUpdateCheck";
const STATE_KEY_DISMISSED = "surrealmemory.dismissedVersion";
...
export async function checkForUpdates(
  ...
  try {
    ...
    const action = await vscode.window.showInformationMessage(
      `Surreal-Memory ${releaseName} is available (current: v${currentVersion}).`,
      "View Release",
      "Dismiss",
    );
...
function getLocalVersion(): string {
  const ext = vscode.extensions.getExtension("neuralmem.surrealmemory");
  if (ext) {
  ...
  try {
    const resp = await fetch(GITHUB_API_URL, {
      headers: {
        ...
        "User-Agent": "surrealmemory-vscode",
      },
