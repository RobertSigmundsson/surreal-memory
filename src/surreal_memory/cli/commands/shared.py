...
def shared_enable(
    server_url: Annotated[
        str, typer.Argument(help="Surreal-Memory server URL (e.g., http://localhost:8000)")
    ],
    api_key: Annotated[
    ...
    Examples:
        ...
        smem shared enable https://memory.example.com --api-key mykey
        smem shared enable http://server:8000 --timeout 60
    """
    config = get_config()
    ...
    typer.secho(
        "Use 'smem shared disable' to switch back to local storage.", fg=typer.colors.BRIGHT_BLACK
    )
...
def shared_test() -> None:
    ...
    if not config.shared.server_url:
        typer.secho(
            "No server URL configured. Use 'smem shared enable <url>' first.", fg=typer.colors.RED
        )
        raise typer.Exit(1)
...
def shared_activate(
    ...
    Examples:
        smem shared activate --key NM-PRO-XXXX-XXXX-XXXX
    """
    ...
    from surreal_memory.unified_config import LicenseConfig, set_config
    from surreal_memory.unified_config import get_config as get_unified_config
