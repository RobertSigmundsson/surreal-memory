...
def project_create(
    ...
    Examples:
        smem project create "Q1 Sprint"
        smem project create "Auth Refactor" --duration 14 --tag backend
        smem project create "Research" -d "ML exploration" --priority 2.0
    """
...
def project_list(
    ...
    if json_output:
        ...
        if not projects:
            typer.echo("No projects found. Create one with: smem project create <name>")
            return
...
def project_delete(
    ...
    Examples:
        smem project delete "Old Project"
        smem project delete "Temp" --force
    """
    ...
    if "error" in result:
        ...
        if result.get("memories_preserved", 0) > 0:
            typer.secho(
                f"  {result['memories_preserved']} memories preserved (use 'smem list' to see them)",
...
def project_extend(
    ...
    Examples:
        smem project extend "Q1 Sprint" 7
    """
