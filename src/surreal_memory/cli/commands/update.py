"""Self-update command for surreal-memory CLI."""
...
from surreal_memory.cli.update_check import _fetch_latest_version, _is_newer

logger = logging.getLogger(__name__)


def _detect_install_mode() -> str:
    """Detect how surreal-memory was installed.
...
def update(
    ...
    Examples:
        smem update              # Update to latest
        smem update --check      # Just check for updates
        smem update --force      # Force reinstall
    """
...
def _update_from_pip(force: bool) -> None:
    ...
    typer.echo("\nUpdating via pip...")
    cmd = [sys.executable, "-m", "pip", "install", "-U", "surreal-memory"]
    if force:
...
def _show_new_version() -> None:
    ...
    returncode, output = _run_command(
        [sys.executable, "-c", "from surreal_memory import __version__; print(__version__)"]
    )
    if returncode == 0 and output:
        typer.secho(f"Now running: surreal-memory {output}", fg=typer.colors.GREEN)
