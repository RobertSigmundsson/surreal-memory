...
class DBTrainHandler:
    ...
    Exposes smem_train_db with actions: train, status.
    ...
    async def _train_db_schema(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        system_tmp = Path("/tmp").resolve()  # noqa: S108  # nosec B108 — czlon ALLOWLISTY walidacji sciezek, nie tworzenie plikow
    ...
    async def _train_db_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "message": (
                ...
                if trained_count > 0
                else "No schema training data. Use smem_train_db(action='train', connection_string='sqlite:///path') to train."
            ),
        }
