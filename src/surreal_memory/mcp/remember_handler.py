...
class RememberHandler:
        ...
        try:
            ...
            try:
                ...
                return (
                    ...
                    "~/.surrealmemory/config.toml with enabled=true, "
                ...
                return (
                    ...
                    "pip install surreal-memory[embeddings], then add [embedding] section to "
                    "~/.surrealmemory/config.toml with enabled=true, "
                    "provider='sentence_transformer', "
                    "model='paraphrase-multilingual-MiniLM-L12-v2'."
                )
    ...
    async def _remember(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Build dedup pipeline if enabled (shared builder — see engine.dedup;
        # same logic now reused by the stop-hook and other encode() callers).
        from surreal_memory.engine.dedup import build_dedup_pipeline

        dedup_pipeline = build_dedup_pipeline(self.config, storage)
