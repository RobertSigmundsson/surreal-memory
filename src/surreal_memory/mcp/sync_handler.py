...
PRO_LANDING_URL = "https://surrealmemory.theio.vn/"
...
class SyncToolHandler:
    ...
    async def _sync(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if action == "seed":
                ...
                return {
                    ...
                    "message": (
                        ...
                        "Run smem_sync(action='push') to push to hub."
                        if total > 0
            ...
            if not self.config.sync.enabled:
                return {
                    "status": "disabled",
                    "message": "Sync is not enabled. Use smem_sync_config(action='setup') to get started.",
                }
            ...
            if not hub_url:
                return {
                    "status": "error",
                    "message": "No hub URL configured. Use smem_sync_config(action='setup') to configure.",
                }

            # Get strategy
            from surreal_memory.sync.protocol import ConflictStrategy
    ...
    async def _try_merkle_sync(
        ...
        try:
            ...
            # Process diffs
            from surreal_memory.sync.protocol import MerkleBucketDiff, MerkleSyncResponse
    ...
    async def _sync_status(self, args: dict[str, Any]) -> dict[str, Any]:
        """Handle smem_sync_status tool call."""
        try:
            storage = await self.get_storage()
    ...
    async def _sync_config(self, args: dict[str, Any]) -> dict[str, Any]:
        """Handle smem_sync_config tool call."""
        try:
            ...
            if action == "setup":
                ...
                return {
                    ...
                    "steps": [
                        ...
                        "4. Run: smem_sync_config(action='set', hub_url='https://YOUR-WORKER.workers.dev', api_key='nmk_YOUR_KEY')",
                        "5. Run: smem_sync(action='seed') to prepare existing memories",
                        "6. Run: smem_sync(action='push') to upload to cloud",
                    ],
...
def _handle_http_error(status: int) -> dict[str, Any]:
    ...
    messages: dict[int, str] = {
        401: "Invalid or expired API key. Run smem_sync_config(action='setup') to reconfigure.",
