...
Provides the smem_tier tool with actions:
...
class TierHandler:
    ...
    async def _tier_evaluate(self) -> dict[str, Any]:
        ...
        if not self.config.is_pro():
            return {
                "error": "Auto-tier requires Pro. Free users can set tiers manually via smem_edit."
            }
    ...
    def _tier_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cold_archive_days": self.config.tiers.cold_archive_days,
            "max_hot_memories": self.config.tiers.max_hot_memories,
            "hint": "Edit ~/.surrealmemory/config.toml [tiers] section to change thresholds.",
        }
