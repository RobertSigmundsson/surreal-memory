...
async def distill_reasoning_patterns(
    ...
    *,
    ...
    async for f in _reasoning_pattern_fibers(storage):
        md = f.metadata
        signature = md.get("_reasoning_signature")
        if signature:
    ...
    # another external naming request. Replay goes through the merge path, so a
    # pattern whose twin already exists folds into it instead of minting a
    # duplicate; signature checks keep a second replay a no-op.
    for index, pattern in enumerate(pending_patterns):
        ...
        if outcome == "created":
            ...
            patterns_merged += 1  # a merge fills no slot, so it spends no model budget
