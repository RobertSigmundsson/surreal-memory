...
async def build_injection_context(
    ...
    def _reusable(f: Any) -> bool:
        """Quality gate from upstream #241 — a pattern must be confident, enabled,
        reusable and above the quality floor to be injected at all. Applied PER
        SOURCE below, so the multi-source doctrine and the gate compose instead of
        one silently disabling the other."""
        md = f.metadata
        return (
            float(md.get("_reasoning_confidence", 0.0) or 0.0) >= rt.min_confidence
            and not bool(md.get("_reasoning_injection_disabled", False))
            ...
            and bool(md.get("_reasoning_reusable", True))
            and float(md.get("_reasoning_quality_score", 1.0) or 0.0) >= rt.injection_min_quality
        )
    ...
    for source in sources:
        candidates = [
            f for f in fibers if f.metadata.get("_source_model") == source and _reusable(f)
        ]
        if not candidates:
            continue
