...
Surreal-Memory using the existing DocTrainer pipeline.

Requires: `pip install surreal-memory[watch]` (watchdog dependency).
"""
...
from surreal_memory.engine.doc_chunker import EXTENDED_EXTENSIONS
...
class FileWatcher:
    """Watches directories and auto-ingests files into Surreal-Memory.
    ...
    async def _process_file(self, file_path: Path) -> WatchResult:
        ...
        try:
            from surreal_memory.engine.doc_trainer import TrainingConfig
    ...
    def start(self) -> None:
        ...
        Requires watchdog: `pip install surreal-memory[watch]`
        """
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install surreal-memory[watch]"
            ) from e
