...
Explicit path (smem_remember) uses sanitize_explicit_content() which strips
...
# Max content length for auto-capture (explicit smem_remember has its own 100K limit)
...
# Surreal-Memory context noise — self-referential metadata that gets re-ingested
# and creates junk neurons like "[concept] json message id"
_NM_CONTEXT_NOISE_RE = re.compile(
    r"^#{1,3}\s*(?:Relevant Memories|Related Information|Relevant Context|Surreal-Memory)\b.*$"
    r"|^\[Surreal-Memory\s*[\u2014\u2013\-].*\]$"  # em-dash, en-dash, hyphen
...
def strip_nm_context_noise(text: str) -> str:
    """Strip Surreal-Memory context wrappers and metadata noise from text.

    Use this to clean text that may contain re-ingested NM output before
    feeding it back into auto-capture. Removes:
    - ## Relevant Memories / ## Surreal-Memory section headers
    - [Surreal-Memory — ...] wrapper lines
...
def sanitize_explicit_content(text: str) -> str:
    """Sanitize content for explicit smem_remember path.
    ...
    - Surreal-Memory context noise (re-ingested NM output headers/bullets)
