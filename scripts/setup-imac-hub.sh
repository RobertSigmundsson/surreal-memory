#!/usr/bin/env bash
# =============================================================================
# Surreal-Memory Hub Server — iMac Setup Script
# =============================================================================
...
#   smem sync-config set --hub-url http://<iMac-IP>:8369 --enabled --auto-sync
#   smem sync push
# =============================================================================
...
LAUNCHD_LABEL="com.surrealmemory.hub"
...
# ── Step 2: Install/Upgrade Surreal-Memory ────────────────────────────────────
info "Installing surreal-memory v${NM_VERSION}..."
$PYTHON -m pip install --upgrade "surreal-memory==${NM_VERSION}" 2>&1 | tail -3

# Verify
INSTALLED=$($PYTHON -c "import surreal_memory; print(surreal_memory.__version__)" 2>/dev/null || echo "FAIL")
if [ "$INSTALLED" != "$NM_VERSION" ]; then
    error "Installation failed. Expected v${NM_VERSION}, got: $INSTALLED"
fi
ok "surreal-memory v${INSTALLED} installed"

# ── Step 3: Verify smem CLI ─────────────────────────────────────────────────
info "Checking smem CLI..."
NMEM_PATH=$(which smem 2>/dev/null || echo "")
if [ -z "$NMEM_PATH" ]; then
    # Try common pip bin paths on macOS
    for p in "$HOME/.local/bin/smem" "$HOME/Library/Python/3.12/bin/smem" "/opt/homebrew/bin/smem"; do
        if [ -x "$p" ]; then
            NMEM_PATH="$p"
            break
        fi
...
if [ -z "$NMEM_PATH" ]; then
    warn "smem not in PATH. You may need to add ~/.local/bin to PATH:"
    warn '  echo '\''export PATH="$HOME/.local/bin:$PATH"'\'' >> ~/.zshrc && source ~/.zshrc'
    NMEM_PATH="$PYTHON -m surreal_memory.cli"
    info "Falling back to: $NMEM_PATH"
else
    ok "smem CLI at $NMEM_PATH"
fi
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>surreal_memory.cli</string>
        <string>serve</string>
        <string>--host</string>
    ...
    <string>${HOME}/.surreal-memory/hub-server.log</string>
    <key>StandardErrorPath</key>
    <string>${HOME}/.surreal-memory/hub-server.error.log</string>
...
if curl -sf "http://127.0.0.1:${HUB_PORT}/health" >/dev/null 2>&1; then
    ...
    warn "  tail -f ~/.surreal-memory/hub-server.log"
    warn "  tail -f ~/.surreal-memory/hub-server.error.log"
fi

# ── Step 8: Configure local sync ────────────────────────────────────────────
info "Configuring local sync on iMac..."
$PYTHON -m surreal_memory.cli sync-config set \
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN} Surreal-Memory Hub Server — Setup Complete${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
