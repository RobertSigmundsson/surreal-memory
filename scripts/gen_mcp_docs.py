...
from surreal_memory.mcp.tool_schemas import get_tool_schemas  # noqa: E402
...
TOOL_CATEGORIES: dict[str, tuple[str, list[str]]] = {
    ...
    "session": (
        "Session & Context",
        ["smem_session", "smem_eternal", "smem_recap"],
    ),
    "provenance": (
        "Provenance & Sources",
        ["smem_provenance", "smem_source"],
    ),
    "analytics": (
        "Analytics & Health",
        ["smem_stats", "smem_health", "smem_evolution", "smem_habits", "smem_narrative"],
    ),
    "cognitive": (
        "Cognitive Reasoning",
        [
    ...
    "training": (
        "Training & Import",
        ["smem_train", "smem_train_db", "smem_index", "smem_import"],
    ),
    "management": (
        "Memory Management",
        [
    ...
    "sync": (
        "Cloud Sync & Backup",
        ["smem_sync", "smem_sync_status", "smem_sync_config", "smem_telegram_backup"],
    ),
    "meta": (
        "Versioning & Transfer",
        ["smem_version", "smem_transplant", "smem_conflicts"],
    ),
}
