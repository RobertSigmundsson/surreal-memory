...
from surreal_memory.cli.main import app  # noqa: E402
...
def _format_command(name: str, cmd: click.Command) -> str:
    ...
    # Syntax
    lines.append("```")
    lines.append(f"smem {name} [OPTIONS]")
    lines.append("```")
    lines.append("")
    ...
    for cat_key, cat_label, cmd_names in CLI_CATEGORIES:
        ...
        for cn in cmd_names:
            if cn in all_commands and not isinstance(all_commands[cn], click.Group):
                anchor = cn.replace(" ", "-")
                lines.append(f"  - [`smem {cn}`](#smem-{anchor})")
