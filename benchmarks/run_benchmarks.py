...
async def bench_ground_truth(k: int = 5) -> tuple[BenchmarkReport, dict[str, dict[str, float]]]:
    """Run ground-truth evaluation: Surreal-Memory vs naive baseline.

    Returns:
        Tuple of (neural_report, baseline_results)
    """
    # Set up Surreal-Memory
    config = BrainConfig(activation_threshold=0.1, max_spread_hops=4, max_context_tokens=500)
    storage = InMemoryStorage()
...
def generate_markdown(
    ...
    if neural_report is not None and baseline_results is not None:
        ...
        sections.append("### Overall (Surreal-Memory vs Naive Baseline)\n")
        headers_gt = ["Metric", "Surreal-Memory", "Naive Baseline", "Winner"]
        rows_gt = []
        for metric_name, neural_val, baseline_val in [
            ...
            winner = "Surreal-Memory" if neural_val >= baseline_val else "Baseline"
            rows_gt.append(
                [
                    metric_name,
                    f"{neural_val:.3f}",
        ...
        sections.append("\n### Per-Category Recall\n")
        headers_cat = ["Category", "Surreal-Memory", "Baseline", "Count"]
        rows_cat = []
...
async def main() -> None:
    ...
    bl = baseline_results.get("overall", {})
    print(
        f"  Surreal-Memory -- P@5={neural_report.mean_precision:.3f}  R@5={neural_report.mean_recall:.3f}  MRR={neural_report.mrr:.3f}  NDCG@5={neural_report.mean_ndcg:.3f}"
    )
    print(
