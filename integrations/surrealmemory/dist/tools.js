/**
 * Surreal-Memory dynamic tool proxy for OpenClaw.
 *
    ...
    return async (toolName, args) => {
        if (!mcp.connected) {
            try {
                ...
                return {
                    error: true,
                    message: `Surreal-Memory auto-connect failed: ${err.message}`,
                };
            }
        }
        try {
...
function mcpToolToOpenClaw(mcpTool, call) {
    return {
        name: mcpTool.name,
        description: mcpTool.description ?? `Surreal-Memory tool: ${mcpTool.name}`,
...
export function createFallbackTools(mcp) {
    ...
    return [
        {
            name: "smem_remember",
            description: "Store a memory in Surreal-Memory. Use this to remember facts, decisions, " +
            ...
            execute: (_id, args) => call("smem_remember", args),
        },
        {
            name: "smem_recall",
            description: "Query memories from Surreal-Memory. Use this to recall past information, " +
            ...
            execute: (_id, args) => call("smem_recall", args),
        },
        {
            name: "smem_context",
            description: "Get recent context from Surreal-Memory.",
            parameters: {
                type: "object",
                properties: {
...
export function createCompatibilityTools(mcp) {
    ...
    return [
        {
            name: "memory_search",
            description: "Search memories (legacy alias for smem_recall). " +
                "Prefer smem_recall for full Surreal-Memory features.",
            parameters: {
                type: "object",
                properties: {
            ...
            execute: (_id, args) => call("smem_recall", { query: args.query, depth: 1 }),
        },
        {
            name: "memory_get",
            description: "Get a memory by ID (legacy alias for smem_recall). " +
                "Prefer smem_recall for full Surreal-Memory features.",
            parameters: {
                type: "object",
                properties: {
            ...
            execute: (_id, args) => call("smem_recall", { query: String(args.id), depth: 0 }),
        },
    ];
}
