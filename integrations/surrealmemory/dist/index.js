/**
 * Surreal-Memory — OpenClaw Memory Plugin
 *
 * Brain-inspired persistent memory for AI agents.
 * Occupies the exclusive "memory" plugin slot.
 *
 * Architecture:
 *   OpenClaw ←→ Plugin (TypeScript) ←→ MCP stdio ←→ Surreal-Memory (Python)
 *
 ...
 * "Surreal-Memory service not running" when OpenClaw registers the plugin
 ...
 *
 * OpenClaw + Telegram injects JSON metadata, Surreal-Memory context blocks,
 * env vars, and system boilerplate into ev.prompt. Passing these raw to
 * smem_recall creates junk neurons like "[concept] json message id".
 *
...
export function stripPromptMetadata(raw) {
    ...
    // 2. Remove Surreal-Memory context sections (## Relevant Memories, etc.)
    //    The |$ ensures sections at end-of-string are also stripped.
    cleaned = cleaned.replace(/^#{1,3}\s*(?:Relevant Memories|Related Information|Relevant Context|Surreal-Memory)[\s\S]*?(?=\n#{1,3}\s|\n\n(?![-•*\s])|$)/gim, "");
    ...
    // 4. Remove [Surreal-Memory — ...] wrapper lines
    cleaned = cleaned.replace(/^\[Surreal-Memory\s*[—–-].*\]$/gm, "");
...
/**
 * Strip Surreal-Memory context noise and metadata from auto-capture text.
 *
 * When agent_end forwards assistant messages to smem_auto, those messages
...
export function sanitizeAutoCapture(raw) {
    ...
    cleaned = cleaned.replace(/^#{1,3}\s*(?:Relevant Memories|Related Information|Relevant Context|Surreal-Memory)\b.*$/gim, "");
    // Strip [Surreal-Memory — ...] wrapper lines
    cleaned = cleaned.replace(/^\[Surreal-Memory\s*[—–-].*\]$/gm, "");
 ...
 * This makes the agent aware of which smem_* tools are available.
 */
function buildToolInstructions(tools) {
    const toolList = tools
        .map((t) => `- ${t.name}: ${t.description.slice(0, 100)}`)
        .join("\n");
    return `Surreal-Memory gives you persistent memory across sessions. Use it proactively — each session starts fresh, so without explicit saves ALL discoveries are lost.

These are TOOL CALLS, not CLI commands. Do NOT run "smem remember" in terminal.

## Available Tools
${toolList}

smem_* is your primary memory system. memory_search/memory_get are legacy aliases for smem_recall.

## WHEN TO RECALL
- New session starts → smem_recall("current project context")
- User references past event → smem_recall("<that topic>")
...
Save with: smem_remember(content="Chose X over Y because Z", type="decision", priority=7, tags=["project", "topic"])
...
- For temporary scratch notes: smem_remember(content="...", ephemeral=true) — auto-expires, never synced.

## SESSION END
smem_auto(action="process", text="<brief session summary>")
...
const plugin = {
    ...
    register(api) {
        ...
        api.logger.info(`Registered ${registeredTools.length} Surreal-Memory tools + ${compatTools.length} compat shims (sync)`);
        // ── Service: MCP process lifecycle ───────────────────
        api.registerService({
            id: "surrealmemory-mcp",
            async start() {
                if (!mcp.connected) {
                    try {
                        await mcp.connect();
                        api.logger.info("Surreal-Memory MCP connected in service.start()");
                        ...
                        try {
                            ...
                            api.logger.info(`Surreal-Memory MCP discovered ${dynamicTools.length} tools`);
                        }
                        catch (err) {
                        ...
                        api.logger.error(`Failed to start Surreal-Memory MCP: ${err.message}`);
                        throw err;
                    }
                }
            },
            async stop() {
                ...
                await mcp.close();
                api.logger.info("Surreal-Memory MCP service stopped");
            },
        });
        ...
        api.on("before_prompt_build", async (event, _ctx) => {
            ...
            if (cfg.autoContext && mcp.connected) {
                ...
                try {
                    ...
                    const raw = await mcp.callTool("smem_recall", {
                        query,
                    ...
                    if (data.answer && (data.confidence ?? 0) > 0.1) {
                        result.prependContext = `[Surreal-Memory — relevant context]\n${data.answer}`;
                    }
                }
                catch (err) {
        ...
        if (cfg.autoConsolidate) {
            api.on("gateway_start", async (_event, _ctx) => {
                ...
                try {
                    await mcp.callTool("smem_consolidate", {
                        strategy: "enrich",
                        compact: true,
                    });
        ...
        // ── Done ────────────────────────────────────────────
        api.logger.info(`Surreal-Memory registered (brain: ${cfg.brain}, ` +
