...
const { Surreal-MemoryMcpClient } = await import("../src/mcp-client.js");
...
describe("Surreal-MemoryMcpClient", () => {
  ...
  describe("constructor", () => {
    it("sets defaults correctly", () => {
      const client = new Surreal-MemoryMcpClient({
        pythonPath: "python3",
        brain: "test-brain",
        logger,
      });
  ...
  describe("connect", () => {
    ...
    it("throws on initialize failure with stderr detail", async () => {
      ...
      proc.stderr.emit("data", Buffer.from("ModuleNotFoundError: surreal_memory"));
  ...
  describe("callTool", () => {
    it("sends correct JSON-RPC and returns text content", async () => {
      ...
      // Call a tool
      const callPromise = client.callTool("smem_stats", {});
  ...
  describe("writeMessage / notify guards", () => {
    it("send rejects when process not available", async () => {
      ...
      await expect(client.callTool("smem_stats", {})).rejects.toThrow(
  ...
  describe("drainBuffer", () => {
    ...
    it("handles multiple messages in one chunk", async () => {
      ...
      const call1Promise = client.callTool("smem_stats", {});
      const call2Promise = client.callTool("smem_health", {});
