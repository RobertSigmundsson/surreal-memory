...
import type { Surreal-MemoryMcpClient } from "../src/mcp-client.js";

function makeMockMcp(
  overrides: Partial<Surreal-MemoryMcpClient> = {},
  ...
  return {
    ...
    close: vi.fn(),
    ...overrides,
  } as unknown as Surreal-MemoryMcpClient;
}
...
describe("tool schemas", () => {
  ...
  describe("smem_remember", () => {
    it("has content as required", () => {
      const schema = getSchema(findTool("smem_remember"));
      expect(schema.required).toContain("content");
    });

    it("has all expected properties", () => {
      const props = getPropNames(findTool("smem_remember"));
  ...
  describe("smem_recall", () => {
    it("has query as required", () => {
      const schema = getSchema(findTool("smem_recall"));
      expect(schema.required).toContain("query");
    });

    it("has all expected properties", () => {
      const props = getPropNames(findTool("smem_recall"));
      expect(props).toEqual(
        expect.arrayContaining([
          "query",
          "depth",
  ...
  describe("smem_context", () => {
    it("has no required fields", () => {
      const schema = getSchema(findTool("smem_context"));
      expect(schema.required ?? []).toHaveLength(0);
    });

    it("has limit and fresh_only properties", () => {
      const props = getPropNames(findTool("smem_context"));
  ...
  describe("smem_todo", () => {
    it("has task as required", () => {
      const schema = getSchema(findTool("smem_todo"));
      expect(schema.required).toContain("task");
    });

    it("has task and priority properties", () => {
      const props = getPropNames(findTool("smem_todo"));
      expect(props).toContain("task");
      expect(props).toContain("priority");
    });
  });

  describe("smem_stats / smem_health", () => {
    it("have empty properties (no params)", () => {
      const statsSchema = getSchema(findTool("smem_stats"));
      const healthSchema = getSchema(findTool("smem_health"));
...
describe("tool execution", () => {
  it("auto-connects when service not connected", async () => {
    ...
    expect(callTool).toHaveBeenCalledWith("smem_remember", { content: "test" });
  });
  ...
  it("returns error when auto-connect fails", async () => {
    ...
    expect(result).toEqual({
      error: true,
      message: "Surreal-Memory auto-connect failed: python not found",
    });
  });
  ...
  it("catches callTool exceptions and returns structured error", async () => {
    ...
    expect(result).toEqual({
      error: true,
      message: "Tool smem_remember failed: connection lost",
    });
  });
  ...
  it("passes correct tool name and args to callTool", async () => {
    ...
    expect(callTool).toHaveBeenCalledWith("smem_remember", {
