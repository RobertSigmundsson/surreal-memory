...
export function stripPromptMetadata(raw: string): string {
  ...
  cleaned = cleaned.replace(
    /^#{1,3}\s*(?:Relevant Memories|Related Information|Relevant Context|Surreal-Memory)[\s\S]*?(?=\n#{1,3}\s|\n\n(?![-•*\s])|$)/gim,
    "",
  );
...
export function sanitizeAutoCapture(raw: string): string {
  ...
  cleaned = cleaned.replace(
    /^#{1,3}\s*(?:Relevant Memories|Related Information|Relevant Context|Surreal-Memory)\b.*$/gim,
    "",
  );
...
const plugin: OpenClawPluginDefinition = {
  ...
  register(api: OpenClawPluginApi): void {
    ...
    api.logger.info(
      `Registered ${registeredTools.length} Surreal-Memory tools + ${compatTools.length} compat shims (sync)`,
    );
        ...
        if (!mcp.connected) {
          try {
            ...
            try {
              ...
              api.logger.info(
                `Surreal-Memory MCP discovered ${dynamicTools.length} tools`,
              );
            } catch (err) {
              api.logger.warn(
            ...
            api.logger.error(
              `Failed to start Surreal-Memory MCP: ${(err as Error).message}`,
            );
            throw err;
          }
        }
    ...
    api.logger.info(
      `Surreal-Memory registered (brain: ${cfg.brain}, ` +
