/**
 * Surreal-Memory MCP Client — JSON-RPC 2.0 over stdio.
 *
 * Spawns `python -m surreal_memory.mcp` and communicates using the
...
const CLIENT_NAME = "openclaw-surrealmemory";
...
export class Surreal-MemoryMcpClient {
  ...
  async connect(): Promise<void> {
    ...
    this.proc = spawn(this.pythonPath, ["-m", "surreal_memory.mcp"], {
    ...
    this.proc.on("exit", (code) => {
      ...
      const hint =
        code === 1
          ? " — check that surreal-memory is installed: pip install surreal-memory"
          : "";
    ...
    try {
      ...
      throw new Error(
        `MCP initialize failed: ${(err as Error).message}${detail}\n` +
          `Verify: ${this.pythonPath} -m surreal_memory.mcp`,
      );
    }
