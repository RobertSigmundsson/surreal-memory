  ...
  return async (
    ...
    if (!mcp.connected) {
      try {
        ...
        return {
          error: true,
          message: `Surreal-Memory auto-connect failed: ${(err as Error).message}`,
        };
      }
    }
  ...
  return [
    {
      name: "smem_remember",
      description:
        "Store a memory in Surreal-Memory. Use this to remember facts, decisions, " +
    ...
    },
    {
      name: "smem_recall",
      description:
        "Query memories from Surreal-Memory. Use this to recall past information, " +
  ...
  return [
    {
      name: "memory_search",
      description:
        "Search memories (legacy alias for smem_recall). " +
      ...
      execute: (_id, args) =>
        call("smem_recall", { query: args.query, depth: 1 }),
    },
    {
      name: "memory_get",
      description:
        "Get a memory by ID (legacy alias for smem_recall). " +
        "Prefer smem_recall for full Surreal-Memory features.",
      parameters: {
        type: "object",
        properties: {
      ...
      execute: (_id, args) =>
        call("smem_recall", { query: String(args.id), depth: 0 }),
    },
  ];
}
