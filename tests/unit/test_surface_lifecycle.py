...
class TestRegenerateSurface:
    ...
    async def test_regenerate_produces_valid_surface(self, tmp_path: Path) -> None:
        ...
        with (
            patch("surreal_memory.surface.lifecycle.SurfaceGenerator") as mock_gen_cls,
            patch("surreal_memory.surface.lifecycle.save_surface_text") as mock_save,
        ):
            mock_gen = AsyncMock()
    ...
    async def test_regenerate_respects_token_budget(self, tmp_path: Path) -> None:
        ...
        with (
            patch("surreal_memory.surface.lifecycle.SurfaceGenerator") as mock_gen_cls,
            patch("surreal_memory.surface.lifecycle.save_surface_text") as mock_save,
            patch("surreal_memory.surface.lifecycle.trim_to_budget") as mock_trim,
        ):
            mock_gen = AsyncMock()
            mock_gen.generate.return_value = surface
            mock_gen_cls.return_value = mock_gen
...
class TestShowSurface:
    ...
    async def test_show_valid_surface(self) -> None:
        ...
        with patch("surreal_memory.surface.resolver.load_surface_text", return_value=text):
            result = await show_surface("testbrain")
    ...
    async def test_show_corrupt_surface(self) -> None:
        ...
        with patch(
            "surreal_memory.surface.resolver.load_surface_text",
...
class TestSurfaceHandler:
    ...
    async def test_surface_show_action(self) -> None:
        """smem_surface(action='show') returns surface info."""
    ...
    async def test_surface_generate_action(self) -> None:
        """smem_surface(action='generate') regenerates and returns info."""
        server = self._make_server()
        surface = _make_surface()
        ...
        with patch(
            "surreal_memory.surface.lifecycle.regenerate_surface",
    ...
    async def test_surface_unknown_action(self) -> None:
        """smem_surface with unknown action returns error."""
        server = self._make_server()
    ...
    async def test_surface_default_action_is_show(self) -> None:
        """smem_surface with no action defaults to show."""
        server = self._make_server()
    ...
    async def test_surface_generate_no_brain(self) -> None:
        """smem_surface generate with no brain returns error."""
        server = self._make_server()
