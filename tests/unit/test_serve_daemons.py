"""Tests for smem serve background daemons: auto-decay, re-index, notifications."""
...
class TestDecayLoop:
    ...
    async def test_decay_runs_on_interval(self) -> None:
        from surreal_memory.server.app import _decay_loop
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            with patch(
                "surreal_memory.engine.lifecycle.DecayManager.apply_decay",
...
class TestReindexLoop:
    ...
    async def test_reindex_runs_on_interval(self, tmp_path: Any) -> None:
        from surreal_memory.server.app import _reindex_loop
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            with patch(
                "surreal_memory.engine.file_watcher.FileWatcher.process_path",
                new_callable=AsyncMock,
                return_value=[mock_result],
            ):
                with patch("surreal_memory.engine.doc_trainer.DocTrainer"):
                    with patch("surreal_memory.engine.watch_state.WatchStateTracker"):
                        call_count = 0
...
class TestNotificationLoop:
    ...
    async def test_webhook_sends_json(self) -> None:
        from surreal_memory.server.app import _send_webhook
    ...
    async def test_notification_health_alert_fires(self) -> None:
        from surreal_memory.server.app import _notification_loop
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            with patch(
                ...
                with patch(
                    "surreal_memory.server.app._send_webhook",
                    new_callable=AsyncMock,
                ) as mock_webhook:
                    call_count = 0
