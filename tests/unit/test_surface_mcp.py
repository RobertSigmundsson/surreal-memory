...
class TestResolver:
    ...
    def test_project_surface_takes_priority(self, tmp_path: Path) -> None:
        ...
        nm_dir = project_dir / ".surrealmemory"
        nm_dir.mkdir(parents=True)
        surface_file = nm_dir / "surface.nm"
        surface_file.write_text("test", encoding="utf-8")

        with (
            patch("surreal_memory.surface.resolver.detect_project_root", return_value=project_dir),
            patch("surreal_memory.unified_config.get_surrealmemory_dir", return_value=tmp_path),
        ):
    ...
    def test_load_surface_text_returns_none_when_missing(self, tmp_path: Path) -> None:
        ...
        with patch(
            "surreal_memory.surface.resolver.get_surface_path", return_value=tmp_path / "nope.nm"
        ):
    ...
    def test_load_surface_text_reads_file(self, tmp_path: Path) -> None:
        ...
        with patch("surreal_memory.surface.resolver.get_surface_path", return_value=nm_file):
            result = load_surface_text("default")
    ...
    def test_save_surface_text_creates_directories(self, tmp_path: Path) -> None:
        ...
        with patch("surreal_memory.surface.resolver.get_surface_path", return_value=target):
    ...
    def test_detect_project_root_finds_git(self, tmp_path: Path) -> None:
        ...
        with patch("surreal_memory.surface.resolver.Path.cwd", return_value=tmp_path):
    ...
    def test_detect_project_root_returns_none_at_home(self, tmp_path: Path) -> None:
        ...
        with (
            patch("surreal_memory.surface.resolver.Path.cwd", return_value=tmp_path),
            patch("surreal_memory.surface.resolver.Path.home", return_value=tmp_path),
        ):
            result = detect_project_root()
            assert result is None
...
class TestServerSurfaceLoading:
    ...
    def test_load_surface_caches_by_brain(self) -> None:
        ...
        with patch(
            "surreal_memory.surface.resolver.load_surface_text", return_value=surface_text
        ) as mock_load:
            # First load
    ...
    def test_load_surface_reloads_on_brain_change(self) -> None:
        ...
        with patch(
            "surreal_memory.surface.resolver.load_surface_text", return_value="brain1 content"
        ) as mock_load:
            server.load_surface("brain1")
            assert mock_load.call_count == 1

        with patch(
            "surreal_memory.surface.resolver.load_surface_text", return_value="brain2 content"
        ) as mock_load:
    ...
    def test_load_surface_returns_empty_when_missing(self) -> None:
        ...
        with patch("surreal_memory.surface.resolver.load_surface_text", return_value=None):
    ...
    async def test_initialize_works_without_surface(self) -> None:
        ...
        assert "Surreal-Memory" in instructions  # Base instructions still present
