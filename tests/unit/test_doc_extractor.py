...
class TestExtractJson:
    ...
    def test_extract_json_object(self, tmp_path: Path) -> None:
        ...
        data = {"name": "Surreal-Memory", "version": "2.0", "features": {"pinned": True}}
        json_file.write_text(json.dumps(data), encoding="utf-8")

        result = extract_to_markdown(json_file)
        assert "# config" in result
        assert "Surreal-Memory" in result
...
class TestEdgeCases:
    ...
    def test_file_too_large(self, tmp_path: Path) -> None:
        ...
        with patch("surreal_memory.engine.doc_extractor._MAX_FILE_SIZE", 10):
