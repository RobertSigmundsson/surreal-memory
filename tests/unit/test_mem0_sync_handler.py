...
from surreal_memory.integration.models import ImportResult, SyncState
from surreal_memory.mcp.mem0_sync_handler import Mem0SyncHandler
from surreal_memory.unified_config import Mem0SyncConfig, UnifiedConfig
...
class TestSwitchBrainValidation:
    def test_valid_brain_name(self, tmp_path: Path) -> None:
        # data_dir MUST be isolated: switch_brain() saves config.toml, and the
        # default data_dir is the user's real ~/.surrealmemory.
...
class TestRunMem0Sync:
    ...
    async def test_cooldown_active_skips(self) -> None:
        ...
        with patch("surreal_memory.integration.sync_engine.SyncEngine") as mock_engine_cls:
    ...
    async def test_cooldown_expired_runs_sync(self) -> None:
        ...
        with patch(
            "surreal_memory.integration.sync_engine.SyncEngine",
...
class TestCreateMem0Adapter:
    def test_platform_adapter(self) -> None:
        with patch.dict("os.environ", {"MEM0_API_KEY": "test-key"}):
            with patch("surreal_memory.integration.adapters.mem0_adapter.Mem0Adapter") as mock_cls:
    ...
    def test_self_hosted_adapter(self) -> None:
        with patch(
            "surreal_memory.integration.adapters.mem0_adapter.Mem0SelfHostedAdapter"
        ) as mock_cls:
