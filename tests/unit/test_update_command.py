"""Tests for the smem update CLI command."""
...
from surreal_memory.cli.commands.update import (
    _get_source_dir,
    _run_command,
)
...
class TestGetSourceDir:
    ...
    def test_finds_source_dir_for_current_project(self) -> None:
        """Source dir detection works for the current Surreal-Memory project."""
...
class TestUpdateCLI:
    ...
    def test_check_only_shows_version(self) -> None:
        ...
        with patch(
            "surreal_memory.cli.commands.update._fetch_latest_version",
            return_value="0.19.0",
        ):
    ...
    def test_update_from_pip(self) -> None:
        ...
        with (
            ...
            patch(
                "surreal_memory.cli.commands.update._detect_install_mode",
                return_value="pip",
            ),
            patch(
                "surreal_memory.cli.commands.update._run_command",
                return_value=(0, "Successfully installed surreal-memory-99.0.0"),
            ) as mock_run,
            patch("surreal_memory.cli.commands.update._show_new_version"),
        ):
            result = runner.invoke(app, ["update"])
            assert result.exit_code == 0
            assert "Updated successfully" in result.output
            cmd = mock_run.call_args[0][0]
            assert "surreal-memory" in cmd
    ...
    def test_update_from_source(self) -> None:
        ...
        with (
            ...
            patch(
                "surreal_memory.cli.commands.update._get_source_dir",
    ...
    def test_check_with_update_available(self) -> None:
        ...
        with patch(
            ...
            assert "To update, run: smem update" in result.output
            assert result.exit_code == 0
