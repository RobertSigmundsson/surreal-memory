...
class TestTelegramConfig:
    ...
    def test_unified_config_from_dict(self) -> None:
        ...
        from surreal_memory.unified_config import TelegramConfig as UnifiedTelegramConfig
...
class TestGetBotToken:
    def test_token_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("SURREAL_MEMORY_TELEGRAM_BOT_TOKEN", "test-token-123")
        assert get_bot_token() == "test-token-123"

    def test_token_missing(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("SURREAL_MEMORY_TELEGRAM_BOT_TOKEN", raising=False)
...
class TestTelegramStatus:
    ...
    async def test_status_configured(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("SURREAL_MEMORY_TELEGRAM_BOT_TOKEN", "test-token")
        ...
        with (
            patch(
                "surreal_memory.integration.telegram.get_telegram_config", return_value=mock_config
            ),
...
class TestTelegramHandler:
    ...
    async def test_backup_no_token(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert "SURREAL_MEMORY_TELEGRAM_BOT_TOKEN" in result["error"]
