...
from surreal_memory.cli.main import _args_request_json, _should_run_update_check, app
...
class TestUpdateNoticeOutput:
    ...
    def test_json_cli_output_stays_clean(self) -> None:
        with (
            patch("sys.argv", ["smem", "check", "plain text", "--json"]),
            patch("surreal_memory.cli.main._warn_if_not_initialized"),
            patch("surreal_memory.cli.update_check.run_update_check_background") as mock_start,
        ):
    ...
    def test_non_json_cli_can_start_background_update_check(self) -> None:
        with (
            patch("sys.argv", ["smem", "check", "plain text"]),
            patch("surreal_memory.cli.main._warn_if_not_initialized"),
            patch("surreal_memory.cli.update_check.run_update_check_background") as mock_start,
        ):
    ...
    def test_update_notice_prints_to_stderr_only(self, capsys) -> None:  # type: ignore[no-untyped-def]
        from surreal_memory.cli.update_check import _print_update_notice

        with patch("surreal_memory.cli.update_check._is_editable_install", return_value=False):
