...
- smem_refine: version increment, content update, failure_mode append, trigger append, caps
- smem_report_outcome: execution count, success rate computation, failure append
...
class TestNmemRefine:
    """Tests for the smem_refine handler."""

    @pytest.mark.asyncio
    async def test_missing_neuron_id(self) -> None:
        server = _make_server()
        result = await server.call_tool("smem_refine", {})
    ...
    async def test_no_changes_provided(self) -> None:
        server = _make_server()
        result = await server.call_tool("smem_refine", {"neuron_id": "fiber-1"})
        assert "error" in result
    ...
    async def test_memory_not_found(self) -> None:
        ...
        result = await server.call_tool(
            "smem_refine", {"neuron_id": "nonexistent", "new_content": "updated"}
        )
        assert "error" in result
    ...
    async def test_wrong_type_rejected(self) -> None:
        ...
        result = await server.call_tool(
            "smem_refine", {"neuron_id": "fiber-1", "new_content": "updated"}
        )
        assert "error" in result
...
class TestNmemReportOutcome:
    """Tests for the smem_report_outcome handler."""

    @pytest.mark.asyncio
    async def test_missing_neuron_id(self) -> None:
        server = _make_server()
        result = await server.call_tool("smem_report_outcome", {"success": True})
        assert "error" in result
        assert "neuron_id" in result["error"]
    ...
    async def test_missing_success(self) -> None:
        server = _make_server()
        result = await server.call_tool("smem_report_outcome", {"neuron_id": "fiber-1"})
        assert "error" in result
    ...
    async def test_memory_not_found(self) -> None:
        ...
        result = await server.call_tool(
            "smem_report_outcome", {"neuron_id": "nonexistent", "success": True}
        )
        assert "error" in result
    ...
    async def test_wrong_type_rejected(self) -> None:
        ...
        result = await server.call_tool(
            "smem_report_outcome", {"neuron_id": "fiber-1", "success": True}
        )
        assert "error" in result
    ...
    async def test_failure_increments_failure_count(self) -> None:
        ...
        result = await server.call_tool(
            "smem_report_outcome", {"neuron_id": "fiber-1", "success": False}
        )
