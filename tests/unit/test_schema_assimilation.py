...
from surreal_memory.engine.schema_assimilation import (
    AssimilationAction,
    AssimilationResult,
...
class TestPostEncodeNeuroHook:
    ...
    async def test_post_encode_calls_assimilate(self) -> None:
        ...
        with patch(
            "surreal_memory.engine.schema_assimilation.assimilate_or_accommodate",
    ...
    async def test_post_encode_interference_wired(self) -> None:
        ...
        with patch(
            "surreal_memory.engine.interference.detect_interference",
            new_callable=AsyncMock,
            return_value=[],
        ) as mock_detect:
            await encoder._post_encode_neuro(anchor)
