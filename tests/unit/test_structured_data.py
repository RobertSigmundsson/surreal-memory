...
class TestStructuredDataEncoderStep:
    ...
    async def test_skip_when_no_structure(
        ...
        from surreal_memory.engine.pipeline_steps import StructuredDataEncoderStep
...
class TestVerbatimPropagation:
    ...
    async def test_fiber_gets_verbatim_flag(self) -> None:
        ...
        from surreal_memory.engine.pipeline import PipelineContext
        from surreal_memory.engine.pipeline_steps import BuildFiberStep
...
class TestCsvProseGuard:
    """_detect_csv_row must not treat prose-with-commas as CSV (audit S-05:
    such misfires created spurious col_N entity-cell neurons that poisoned the
    graph)."""
    ...
    def test_prose_with_commas_and_a_date_not_csv(self):
        ...
        prose = (
            "Session activity: wejdz w role ekspert prompt loop enineer, "
            "architektury systemow orkiestracji, sweep 2026-06-26"
        )
