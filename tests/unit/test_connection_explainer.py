"""Tests for smem_explain — connection explainer feature."""
...
from surreal_memory.engine.connection_explainer import (
...
class TestConnectionMCPHandler:
    """Tests for smem_explain MCP tool integration."""
    ...
    async def test_explain_via_handle_message(self, server: MCPServer) -> None:
        """smem_explain should work through JSON-RPC handle_message."""
        mock_storage = AsyncMock()
        mock_storage.find_neurons = AsyncMock(return_value=[])
        mock_brain = MagicMock()
        mock_brain.id = "test-brain-id"
