...
def mock_server():
    ...
    server.get_tools.return_value = [{"name": "smem_recall"}]
...
class TestHTTPTransport:
    ...
    def test_mcp_get_info(self, client):
        ...
        assert data["server"] == "surreal-memory-mcp"
    ...
    def test_initialize(self, client):
        ...
        data = resp.json()
        assert data["id"] == 1
        assert data["result"]["serverInfo"]["name"] == "surreal-memory"
    ...
    def test_main_parses_http_flag(self):
        ...
        with (
            patch("surreal_memory.mcp.server.asyncio") as mock_asyncio,
            patch("surreal_memory.mcp.http_transport.run_http_server"),
        ):
            ...
            old_argv = sys.argv
            try:
                sys.argv = ["smem-mcp", "--http"]
                from surreal_memory.mcp.server import main
    ...
    def test_main_default_is_stdio(self):
        ...
        with patch("surreal_memory.mcp.server.asyncio") as mock_asyncio:
            mock_asyncio.run = lambda coro: None
            import sys
